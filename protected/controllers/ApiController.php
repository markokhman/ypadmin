    ...
    class ApiController extends Controller
    {
        ...
        {     
            ...
            switch ($category) {
                ...
                case 1000:
                    ...
                    //$models = Client::model()->findAllBySql ('SELECT * FROM client WHERE cat_id IN ('.$cat1.')');               
                    break;
                default:
        ...
        {        
            ...
            if($user->gender == "женский")
                ...
                $gen = 1;
            $markTotalAmount = $this->marks($uid, 0, 1);
            //$models = Notification::model()->findAllBySql('select c.cid, c.nid, c.message, c.age_min, c.age_max, c.gender, c.attend_max, c.attend_min, c.mark_max, c.mark_min, c.mark_here, c.star_max, c.star_min from notification c, user u where u.uid='.$uid.' AND (c.age_min <= DATEDIFF("'.$aaa.'", u.dateofbirth)/365 AND c.age_max >= DATEDIFF("'.$aaa.'", u.dateofbirth)/365) AND c.gender IN (0, '.$gen.')');        
            ...
            switch($_GET['model'])
            {
                ...
                case 'company':
                    ...
                    if($user->gender == "???????")
                        ...
                        $gen = 1;
                    //$certs = Certificate::model()->findAllByAttributes(array('cid'=>$id));
        ...
        {        
/*            $id = $this->_checkAuthClient();                    
            if($id == '-1'){
                $this->_sendResponse(200, 'Invalid password or email');
            }
            else{*/
                $model = new Client;
...
/*            $id = $this->_checkAuthClient();        
            if($id < 0){
                $this->_sendResponse(200, 'Invalid password or ID'.$id);
            }
            else{*/
                $model = new Client;
                $model->email = $_POST['email'];
